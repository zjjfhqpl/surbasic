...
// 6.9.1 If the output of the algorithm given in A.12.1 is 乬False,乭 then 
...
// Key Derivation Function 2 (SurDoc version)
// 服务器端生成密钥的时候只生成了20字节，也就是MAC密钥只有前4字节有效，后12字节为0
OCTETSTR KDF2_SURDOC (OCTETSTR Z, OCTETSTR::size_type oLen, OCTETSTR P) {
	//if (oLen > 160*((2<<31)-1)) // 160 * (2^32-1)
	//	throw borzoiException ("KDF2: oLen > hbits * (2^32-1) bits");
	if (oLen > 20*((2<<31)-1)) // 160/8 * (2^32-1)
		throw borzoiException ("KDF2: oLen > hbits/8 * (2^32-1) bits");
