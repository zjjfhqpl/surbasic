...
private:
	...
	mp_int mp;
#endif
	friend std::string BigIntToDecimal(const BigInt &val);
public:
	BigInt ();
	BigInt (OCTET o);
	~BigInt ();
...
inline BigInt concat (BigInt a, BigInt b) { BigInt c=a; c<<=
  static_cast<unsigned short>(b.numBits()); c|=b; return c; }
